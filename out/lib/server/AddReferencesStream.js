...
var _Atviseproject = _interopRequireDefault(require("../config/Atviseproject"));
...
const serverNodes = new Set(_Atviseproject.default.ServerRelatedNodes.map(id => id.value));
 ...
 */
class AddReferencesStream extends (0, _WaitingStream.waitForDependencies)(_CallScriptStream.default) {
  /**
  ...
  dependenciesFor(file) {
    ...
    return Object.values(refs).reduce((deps, nodes) => deps.concat(nodes), []).filter(({
      value
    }) => !serverNodes.has(value));
  }
   ...
   */
  get scriptId() {
    return new _NodeId.default(_NodeId.default.NodeIdType.STRING, 'SYSTEM.LIBRARY.ATVISE.SERVERSCRIPTS.atscm.AddReferences', 1);
  }

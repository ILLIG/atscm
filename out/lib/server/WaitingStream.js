...
exports.waitForDependencies = exports.default = void 0;
 ...
 * }
 */
const waitForDependencies = Stream => class Waiting extends Stream {
  /**
  ...
  _enqueueChunk(file) {
    const dependencies = this.dependenciesFor(file).filter(id => {
      if (id.namespace === 0 || !id.value || _ProjectConfig.default.nodes.includes(id)) {
        return false;
      }
  ...
  _flush(callback) {
    const checkProcessing = () => {
      if (this._processing || this.hasPending) {
        ...
        _gulplog.default.debug(`Missing dependency. Trying to process dependents of ${first}`);
        this.once('finished-chunk', () => checkProcessing());
        this.emit('processed-chunk', {
          nodeId: first
        });
      } else {
...
class WaitingStream extends waitForDependencies(_QueueStream.default) {}

...
exports.default = exports.TransformDirection = void 0;
 ...
 */
const TransformDirection = exports.TransformDirection = {
  FromDB: 'FromDB',
...
class Transformer {
  ...
  compatTransform(direction, node, context) {
    const transform = (direction === TransformDirection.FromDB ? this.transformFromDB : this.transformFromFilesystem).bind(this);
    const fnName = `${this.constructor.name}#transform${direction === TransformDirection.FromDB ? 'FromDB' : 'FromFilesystem'}`;
    return new Promise((resolve, reject) => {
      const promise = transform(node, context, (err, result) => {
        if (!this.constructor._warnedStreamAPI) {
          ...
          _gulplog.default.debug(`Deprecated: ${fnName} uses the Stream API instead of async/await.`);
        }
        if (err) {
      ...
      if (promise instanceof Promise) {
        promise.then(resolve, err => reject(Object.assign(err, {
          node
        })));

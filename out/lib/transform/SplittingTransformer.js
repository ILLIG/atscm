...
class SplittingTransformer extends _PartialTransformer.default {
   ...
   * @param {Node | BrowsedNode} node The file to split.
   * @param {?string} newExtension The extension the resulting file gets.
   * @return {Node | BrowsedNode} The resulting node.
   */
  ...
  async transformFromFilesystem(node, context) {
    ...
    const regExp = new RegExp(`^\\.${(0, _regexp.escapeForRegExp)(name)}(${this.constructor.sourceExtensions.join('|')})\\.json$`);

    // Find source files an child definition files
    const sourceFiles = [];
    const childFiles = [];
    const children = (await (0, _fsExtra.readdir)(node.relative)).reduce((current, f) => {
      if (f.match(regExp)) {
        sourceFiles.push(f);
    ...
    const sourceNodes = await Promise.all(sourceFiles.map(f => context.readNode({
      path: (0, _path.join)(node.relative, f),
      tree: {
        parent: node
      }
